...
class WebGraph():
    ...
    def power_method(self, v=None, x0=None, alpha=0.85, max_iterations=1000, epsilon=1e-6):
        ...
        with torch.no_grad():
            ...
            nondanglingNode = torch.sparse.sum(self.P,1).indices()
            a = torch.ones([n,1])
            a[nondanglingNode] = 0
            ...
            for i in range(max_iterations):
                ...
                x /= torch.norm(x, p=1)
                residual = torch.norm(x-x_prev)
                logging.debug('i=' +str(i)+' residual= '+str(residual)+' sum(x)= '+str( torch.sum(x)))
                if residual < epsilon:
                    break
            return x.squeeze()
